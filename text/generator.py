...
def otevrit_w():
    ...
    smer_dictionary = filedialog.askopenfilename(title=u"open file",filetypes=(('texts', '*.txt'),('anything', '*')))
    if smer_dictionary != '':
        update_w()
...
def otevrit_p():
    global paterns
    global smer_paterns
    smer_paterns = filedialog.askopenfilename(title=u"open file",filetypes=(('texts', '*.txt'),('anything', '*')))
    if smer_paterns != '':
        update_p()
...
def otevrit_a():
    ...
    smer_additions = filedialog.askopenfilename(title=u"open file",filetypes=(('texts', '*.txt'),('anything', '*')))
    if smer_additions != '':
        update_a()
...
hlavni.title('text generator')
hlavni.iconbitmap("text generator.ico")
...
speedMenu.add_command(label="4",command=speed4)
